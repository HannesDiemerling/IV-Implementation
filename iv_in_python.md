...
Assume a researcher, let’s call her Annabelle, wants to find out if there is a difference in a number of chemical features between three types of wines called Barolo, Lugana, and Primitivo \footnote{where the different types of wine could be different treatment groups in a psychological experiment, different gender identification in a social science study, or different conditions in a neurological study}. Her data set consists of the chemical features for XXX different Barolo, XXX Lugana and XXX Primitivo wines\footnote{The data set of this example is available at XXX [@citeXXX]}. Assuming all three types of wine are identical with respect to the chemical features under investigation, no classifier would be able to perform better than guessing, which corresponds to a BAC of $\frac{1}{3}$. Annabelle’s research hypothesis in terms of the BAC is hence whether the BAC is above $\frac{1}{3}$ for a classifier that asymptotically should be able to pick up the difference. 
She chooses a Support Vector Machine (SVM) as her classifier, imports the IV package and initiates the IV object with this classifier and her data set. She chooses the initial training set size of five and the default batch size of one since her data set is rather small. With this, she runs the iv process. She then starts the estimation process that gives a posterior distribution for $a$ and $b$, using a burn-in phase of 100 samples, a thinning of 10, and the default step size 0.1 she generates 1000 samples from the posterior. Annabelle can now request different numbers, distributions and plots from this object. To answer her research question, she is interested in the posterior probability using a flat prior that the BAC exceeds $\frac{1}{3}$, which she can access directly from the object; she obtains a posterior probability of XXX%, that is, with XXX% probability the classifier is able to differentiate between the classes. She concludes that there are at least some meaningful differences between the types of wine.
To illustrate the results for her reader Annabelle plots the distribution of the BAC. She also reports the MAP of the distribution which she can get directly from her iv object, it is XXX and the 95% credible interval which she too can get from the IV object. It goes from XXX to XXX.  This shows that $\frac{1}{3}$ is not within the credible interval making it clear that there is a difference between the wines. Specificly the chance for the BAC to be less than or equal to $\frac{1}{3}$ is the area under the curve below that point. In the plot it is visible that this area is clearly very low, specificly XXX.  This result clearly rejects the hypothesis of no group difference. [Hier noch Plotbeschreibung hinzufügen] [Plot der posterior BAC].
She is now also interested in the next question whether two of her wines, Barolo and Lugana, are substantially different in their chemical composition. For this she takes a subset of her dataset with only these two wines. She then creates a new IV object and runs the compute posterior method, using the same parameters as before. Annabelle is still using the SVM, now as a binary test to find if a certain wine is a Barolo or not. She can extract the sensitivity of this test by getting the accuracy within the Barolo class and the specificity by taking the accuracy within the Lugana class. Both can directly be gotten from the IV by specifying the class label.

## Which classifier is best?
Although the SVM provided strong classification results, Annabelle is curious whether other classifiers would have performed equally well. So she creates three more iv objects with different classifers (K-Nearest Neighbor, Random Forest and Linear Regression). She then runs the IV and computes the posterior for each. The value she wants to optimize is the BAC, therefore this is what she plots for each. Comparing them shows that the SVM performed rather worse than some of the others. [BAC Plots] The linear regression XXX and the Random Forest XXX both performed similarly. To find which one is best and how much better than the other she calculates the probability that a random drawing from the BAC of the linear regression XXX is higher than a random drawing from the random forest XXX. A service function is contained in the package where Annabelle can input her distributions and will receive this probability. It comes out to XXX. [Vergleich von Classifiern, Posterior Wahrscheinlichkeit, dass einer besser ist als der andere]. 
A friend of Annabelles wants to create an actual tool that is able to guess which type of wine it samples. He gets paid by correctly classified wines, so he is not interested in the BAC, but in the global accuracy (i.e., the percentage correctly classified). To assist him, Annabelle investigates which classifier performs best for the ACC. She gets the MAPs for the ACC distributions for each of her classifiers and plots the full distribution for the best, being XXX.  Therefore she recommends this classifier to her friend. [ACC plot statt BAC]. 
Annabelle’s friend wants to use as few wines as possible for his finite training to avoid costs. He asks how many wines he should purchase; to help him, Annabelle investigates the impact of an increasing training set on the accuracy. For this she gets the development of the ACC over training size. She makes the IV object plot the means as well as the lower and upper quartile of the ACC distribution for every training set size from 1 to 100 (default 100). She plots this and sends it to her friend with the explanation that the classifiers performance increases strongly for the first XXX samples and then continues increasing thereby approaching an asymptotical value. [Plot n gegen ACC]. 
XXX Maybe explain get development in output section ?
When marketing the tool, her friend realizes that his customers are not so much interested in the accuracy of the classifier, but rather if the classifier is able to recognize Lugana wine precisely. Annabelle helps out her friend again by checking the accuracy of the classifer on Lugana wine. Within this class the accuracy is XXX. 
